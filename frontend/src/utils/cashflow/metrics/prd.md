...
- ☐ MR-1 Create `frontend/src/utils/cashflow/metrics/registry.js` - unified CASHFLOW_METRICS_REGISTRY
- ☐ MR-2 Create individual metric calculation files with standardized exports
...
- ☐ MI-1 Create `frontend/src/utils/cashflow/metrics/calculations/irr.js` with calculate, format, threshold functions
- ☐ MI-2 Create `frontend/src/utils/cashflow/metrics/calculations/npv.js` with standardized exports
- ☐ MI-3 Create `frontend/src/utils/cashflow/metrics/calculations/dscr.js` for debt service coverage
...
- ☐ MI-9 Create `frontend/src/utils/cashflow/metrics/calculations/index.js` with wildcard exports using * imports
...
- ☐ FC-3 Migrate `createFinancialMetricsConfig()` to use unified metrics registry
- ☐ FC-4 Remove deprecated references to old `financingMetrics.js` functions
- ☐ FC-5 Update MetricsDataTable configuration to use standardized metrics data
...
- ☐ DE-3 Integrate with new calculation engine for sensitivity analysis
- ☐ DE-4 Update `createDriverAnalysisFooter()` to use registry-based metric definitions
- ☐ DE-5 Remove hardcoded metric references and use registry discovery

### Phase 4: Legacy Cleanup 🔥 🏷️Medium
- ☐ LC-1 Audit and remove duplicate functions across `metricsUtils.js`, `financingMetrics.js`, `finance/calculations.js`
- ☐ LC-2 Update all references to use new unified calculation engine functions
- ☐ LC-3 Remove static WIND_INDUSTRY_AGGREGATIONS in favor of registry-based aggregation strategies
- ☐ LC-4 Clean up inconsistent function naming and standardize on unified patterns
- ☐ LC-5 Remove unused imports and dependencies from existing files
- ☐ LC-6 Update all import statements across codebase to use new unified system
- ☐ LC-7 Remove all old metric calculation patterns and unused functions
- ☐ LC-8 Remove `METRIC_CALCULATORS` from `metricsUtils.js`
- ☐ LC-9 Clean up unused imports and dependencies

### Phase 5: Documentation & Testing 📚 🏷️Medium
- ☐ TD-1 Create comprehensive tests for new calculation engine and metrics registry
- ☐ TD-2 Create `frontend/src/utils/cashflow/metrics/documentation.md` with usage patterns
- ☐ TD-3 Add JSDoc documentation for all new functions and registry structures
- ☐ TD-4 Create migration guide for adding new metrics to the system
- ☐ TD-5 Document performance optimization strategies and caching behavior
- ☐ TD-6 Add comprehensive JSDoc comments to all functions
- ☐ TD-7 Create unit tests for each individual metric calculation
- ☐ TD-8 Create integration tests for registry and processor utilities
- ☐ TD-9 Create end-to-end tests for card functionality with new system
